...
**解释模式**适合你遇到陌生内容的情况——某个术语、概念、缩写，或者一段读不太懂的文字。划选后点击 ✦，AI 立即结合上下文进行解释，不需要你自己组织问题。

**提问模式**适合你已经理解文字本身的意思，但有具体想法想进一步探讨的情况——比如：*"这个论点逻辑上成立吗？"*、*"这个观点有什么反驳意见？"*、*"能给我一个现实中的例子吗？"*、*"这和 X 有什么区别？"*。划选相关段落，点击 **?**，直接输入你想问的问题。
...
Context Explain 完整渲染 AI 输出的各种格式：

- **Markdown** — 标题、加粗/斜体、表格、有序/无序列表、引用块、行内代码、代码块（含语言标签，由 [marked.js](https://marked.github.io/marked/) 提供）
- **数学公式** — 通过 [KaTeX](https://katex.org/) 渲染行内和块级 LaTeX，支持 `\(...\)` · `\[...\]` · `$$...$$` 格式。查询线性规划问题时，公式会正确排版：$3x_1 + 5x_2 \leq 12$
...
| 📝 **Markdown** | 完整 GFM（marked.js）— 表格、任务列表、代码块、删除线 |
| 🖊️ **页面标注** | 琥珀色下划线，刷新后自动恢复 |
| 🗺️ **滚动条标记** | 全页面标注位置小地图 |
| 🪟 **可拖动面板** | 自由移动，按上下文记忆位置 |
| 📚 **查询历史** | 搜索 · 导出 JSON · 单条删除 · 详情页完整渲染 |
